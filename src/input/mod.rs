...
impl State {
    ...
    pub fn handle_bind(&mut self, bind: Bind) {
        ...
        if self.niri.is_locked()
            && !(bind.allow_when_locked || bind.action.iter().any(allowed_when_locked))
        {
            return;
        }
                    ...
                    let (bind_left, bind_right) =
                        if should_handle_in_overview && modifiers.is_empty() {
                            let bind_left = Some(Bind {
                                ...
                                action: vec![Action::FocusColumnLeftUnderMouse],
                                repeat: true,
                                cooldown: None,
                            ...
                            let bind_right = Some(Bind {
                                ...
                                action: vec![Action::FocusColumnRightUnderMouse],
                                repeat: true,
                                cooldown: None,
                                allow_when_locked: false,
                                allow_inhibiting: false,
                ...
                if ticks != 0 {
                    let (bind_up, bind_down) = if should_handle_in_overview && modifiers.is_empty()
                    {
                        let bind_up = Some(Bind {
                            ...
                            action: vec![Action::FocusWorkspaceUpUnderMouse],
                            repeat: true,
                        ...
                        let bind_down = Some(Bind {
                            ...
                            action: vec![Action::FocusWorkspaceDownUnderMouse],
                            repeat: true,
                            cooldown: Some(Duration::from_millis(50)),
                            allow_when_locked: false,
                            allow_inhibiting: false,
...
fn should_intercept_key<'a>(
    ...
    if screenshot_ui.is_open() {
        ...
        if let Some(bind) = &final_bind {
            if bind.action.iter().any(allowed_during_screenshot) {
...
mod tests {
    ...
    fn bindings_suppress_keys() {
        ...
        assert!(match filter {
            FilterResult::Intercept(Some(Bind { action, .. })) =>
                action == vec![Action::CloseWindow],
            _ => false,
        });
    ...
    fn comp_mod_handling() {
        let bindings = [
            ...
            Bind {
                ...
                action: vec![Action::FocusColumnLeft],
                repeat: true,
                cooldown: None,
                allow_when_locked: false,
                allow_inhibiting: true,
            ...
            Bind {
                ...
                action: vec![Action::FocusWindowDown],
                repeat: true,
                cooldown: None,
                allow_when_locked: false,
                allow_inhibiting: true,
            ...
            Bind {
                ...
                action: vec![Action::FocusWindowUp],
                repeat: true,
                cooldown: None,
                allow_when_locked: false,
                allow_inhibiting: true,
            ...
            Bind {
                ...
                action: vec![Action::FocusColumnRight],
                repeat: true,
                cooldown: None,
                allow_when_locked: false,
                allow_inhibiting: true,
